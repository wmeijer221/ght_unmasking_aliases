...
dataPath = os.path.abspath('./output/')
if not os.path.exists(dataPath):
    os.makedirs(dataPath)

# TODO: This should probably support unicode again somehow.
w_log = csv_writer(open(os.path.join(dataPath, 'idm_log.csv'), 'w+'))
writer = csv_writer(open(os.path.join(dataPath, 'idm_map.csv'), 'w+'))
w_maybe = csv_writer(open(os.path.join(dataPath, 'idm_maybe.csv'), 'w+'))
...
reader = open(os.path.join(dataPath, 'users_emails_sample.csv'), 'r')
# _header = reader.next()
...
for (a, b), list_clues in sorted(list(clues.items()), key=lambda e: (int(e[0][0]), int(e[0][1]))):
    aa = aliases[a]
    ab = aliases[b]
